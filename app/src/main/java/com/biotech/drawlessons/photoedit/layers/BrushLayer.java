...
import com.biotech.drawlessons.photoedit.draws.NormalLineBrush;
 ...
 * 笔刷的 layer，这个 layer 处理这样的笔刷：
 * touchDown的时候-->创建笔刷（最基础的创建，不需要shader，所以backgroundLayer不适合）
 * touchMove的时候-->在屏幕上绘制出这个笔刷（drawingBoard传过来的canvas）（所以lightLine不适合这个layer）
 * touchUp的时候-->保存笔刷，并且把笔刷绘制到internalBitmap上（所以马赛克不适合这个layer）
 */

public class BrushLayer extends BaseLayer {
    ...
    public BrushLayer(ILayerParent parent, DrawInvoker invoker, Matrix matrix, BitmapsManager manager, DimensionManager dimensionManager) {
        super(parent, IPhotoEditType.LAYER_BRUSH, matrix);
     ...
     * 最普通的颜色画笔模式
     * */
    public void setNormalColorMode(int color) {
        mType = IPhotoEditType.BRUSH_NORMAL_COLOR;
        mColor = color;
    }
     ...
     * 贴纸画笔模式
     * */
    public void setStickerBrushMode(String stickerUri) {
        mStickerUri = stickerUri;
        mType = IPhotoEditType.BRUSH_STICKERS;
    }

    private NormalLineBrush createNormalLineBrush() {
        return new NormalLineBrush(mMatrix, mColor, mDimensionManger.getStrokeWidth(), mPaint);
    }

    public NormalLineBrush createNormalLineBrush(BaseDrawData data) {
        return NormalLineBrush.createBrush(data, mMatrix, mPaint);
    }

    public StickerBrush createStickerBrush(BaseDrawData data) {
        return StickerBrush.createStickerBrush(data, mMatrix, mPaint, mBitmapManager);
    }

    private StickerBrush createStickerBrush() {
        return new StickerBrush(mMatrix, mStickerUri, mPaint, mBitmapManager);
    }
