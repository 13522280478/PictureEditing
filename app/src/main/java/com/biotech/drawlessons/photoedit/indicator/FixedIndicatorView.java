package com.biotech.drawlessons.photoedit.indicator;
...
import com.biotech.drawlessons.photoedit.indicator.slidebar.ScrollBar;
...
public class FixedIndicatorView extends LinearLayout implements Indicator {
	...
	public void initNotifyOnPageScrollListener() {
		int tabCount;
			...
			if (onPageScrollListener != null && tabCount > 1) {
				...
				for(int i=0;i<tabCount;i++){
					if(i!=mSelectedTabIndex){
						View view1 = getItemView(i);
						if (view1 != null) {
							onPageScrollListener.onTransition(view1, i, 0);
						}
					}
				}
			}
...
//			notifyPageScrolled(position, positionOffset, positionOffsetPixels);
		} else if (mPositionOffset > 0.0001f) {
